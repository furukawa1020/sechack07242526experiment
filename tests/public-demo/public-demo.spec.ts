...
test("公開レビュー用の固定経路を実機なしで開き、同じブラウザの表示だけを同期する", async ({
  ...
  await displayPage.goto("/display/demo/", { waitUntil: "networkidle" });
  ...
  await page.goto("/device-test/", { waitUntil: "networkidle" });
  ...
  await page.goto("/healthz/", { waitUntil: "networkidle" });
