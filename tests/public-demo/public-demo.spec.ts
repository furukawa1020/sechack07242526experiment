...
async function expectDemoNoticeFullyVisible(page: Page): Promise<void> {
  ...
  const boxes = await notice.locator("strong, span").evaluateAll((elements) =>
  ...
  await expect(notice).toContainText("研究参加用ではありません");
}
...
async function expectSafeVisibleState(page: Page): Promise<void> {
  ...
  await expect(page.locator(".public-demo-notice")).toContainText(
    "固定模擬データ・本人非測定・生体データ取得なし",
  );
...
test("固定模擬データの4提示を操作でき、入力・フォーム・内部コードを公開しない", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "同じ固定模擬データを、4つの方法で提示します" }),
  ).toBeVisible();
  await expect(page.getByRole("main")).toHaveCount(1);
  await expect(page.getByRole("main").getByRole("heading", { level: 1 })).toHaveText(
    "同じ固定模擬データを、4つの方法で提示します",
  );
  await expect(page.getByText("表示される値は、あなた自身を測定したものではありません。")).toBeVisible();
  ...
  for (const presentation of presentations) {
    await expect.poll(() => page.evaluate(() => window.scrollY)).toBe(0);
    await expectDemoNoticeFullyVisible(page);
    ...
    await page.getByRole("button", { name: "次へ" }).click();
    const response = page.getByTestId("public-demo-response");
    await expect(response).toBeVisible();
    await expect(response).toHaveText(
      `第${presentation.position}提示は終了しました研究スタッフの案内をお待ちください。`,
    );
    await expect(response.locator("h1")).toHaveText(
      `第${presentation.position}提示は終了しました`,
    );
    await expect(response.locator("p")).toHaveText("研究スタッフの案内をお待ちください。");
    await expect(page.locator(".public-demo-comparison")).toHaveCount(0);
...
test("自動リハーサルが規定時間で4提示を再生し、通信や保存を行わない", async ({ page }) => {
  ...
  for (let position = 1; position <= 4; position += 1) {
    ...
    await expect(stage).toHaveAttribute("data-rehearsal-phase", "response");
    await expect(page.getByTestId("public-demo-response")).toHaveText(
      `第${position}提示は終了しました研究スタッフの案内をお待ちください。`,
    );
    const confirmButton = page.getByRole("button", { name: "待機表示を確認して次へ" });
...
test("公開レビュー用の固定経路を実機なしで開き、同じブラウザの表示だけを同期する", async ({
  ...
  await expect(
    displayPage.getByText("進行画面との接続を待っています。同じブラウザで進行画面を開いてください。"),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "第1提示", exact: true }).click();
  ...
  await page.getByRole("button", { name: "第2提示", exact: true }).click();
  await expect.poll(() => displayPage.evaluate(() => window.scrollY)).toBe(0);
  await expectDemoNoticeFullyVisible(displayPage);
  await expect(displayPage.locator(".public-demo-presentation-header")).toContainText(
    "第2提示 / 4",
  );
  await expect(
...
test("タブ間同期を生成できないブラウザでは手動デモへ安全に案内する", async ({ page }) => {
  const network = monitorNetwork(page);
  await page.addInitScript(() => {
    Object.defineProperty(window, "BroadcastChannel", {
      configurable: true,
      value: class UnsupportedBroadcastChannel {
        constructor() {
  ...
  await page.goto("/operator/index.html", { waitUntil: "networkidle" });
  await expect(page.getByText(/タブ間同期を利用できません/u)).toBeVisible();

  await page.goto("/display/demo/index.html", { waitUntil: "networkidle" });
  await expect(page.getByText(/タブ間同期を利用できません/u)).toBeVisible();
  const manualLink = page.getByRole("link", { name: "手動デモを開く" });
  await expect(manualLink).toBeVisible();
  await manualLink.click();
  await expect(page.locator(".public-demo-notice")).toContainText("研究参加用ではありません");
  const manualDemoUrl = new URL(page.url());
  expect(manualDemoUrl.origin).toBe(EXPECTED_PUBLIC_DEMO_ORIGIN);
  // Vite preview normalizes index.html to `/`; Static Space keeps `/index.html`.
  expect(["/", "/index.html"]).toContain(manualDemoUrl.pathname);
