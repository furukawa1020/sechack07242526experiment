...
`R8-010-2x2-screen-v3`の正式MVPは`device.mode=screen`を使用し、参加者画面内のフグだけを動かす。USBシリアル機器を接続せず、デバイス制御のための外部通信も行わない。
...
`mock`を正式実施の代替として使用してはならない。`serial`を`R8-010-2x2-screen-v3`へ混在させてはならない。物理フグと画面上フグの切替は研究刺激の変更であるため、研究責任者の承認と、所属機関で必要な倫理審査・変更手続きを完了してから別プロトコルとして実施する。
...
以下は将来の物理フグ版で維持する通信・安全要件であり、`R8-010-2x2-screen-v3`の正式実施では使用しない。
