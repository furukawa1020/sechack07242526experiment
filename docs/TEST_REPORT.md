...
次の5コマンドを、最終コード変更を含む同一の候補作業ツリーから実行する。過去版の成功結果や部分実行はscreen-v3の合格記録として流用しない。
...
- `formUrl=""`
- `formAudit`が存在しない
- `goEvidence`、承認資料、承認文書ハッシュ、確認者情報が正式設定・manifest・ログに存在しない
- 正式runtimeから外部originへのrequestが0件

screen-v1の外部回答監査はscreen-v3のrelease/start gateとして試験しない。

### EXTERNAL COMPLIANCE MODE

- 承認PDF、承認文書、承認文書ハッシュ、二人目の確認者がなくてもexternal compliance設定を起動できる
- screen pilot件数が0でも、それだけを理由にpreflight・release・startを拒否しない
- Operatorセッション内確認が未完了なら第1提示を開始できない
- 参加者ごとの同意確認が未完了なら第1提示を開始できない
- Operatorセッション内確認を実験ログ、データベース、`localStorage`へ永続保存しない
- アプリまたはブラウザ再起動後にOperator確認を再要求する
- 緊急停止はOperator確認前を含む全状態で機能する
- 参加者画面に承認状態、Operator確認状態、確認者情報を表示しない
- ログに氏名、メール、IP、確認者情報、承認文書ハッシュを記録しない
...
screen pilotは任意の品質確認であり、未実施または件数0でも正式release/startのハードエラーにしない。実施する場合は研究チームの非参加者だけを対象とする。
...
- source commit等を使用する場合は技術的な再現性確認だけに用い、倫理承認証跡として扱わない
...
1. 最終5コマンド、本番preflight、現地screen試験をPASSさせる。
2. `participantMode=enabled`とexternal compliance設定を確認する。
3. 当日のOperatorセッション内確認を完了する。
4. 参加者ごとの提示前同意をアプリ外で確認する。
5. 緊急停止が利用可能であることを確認する。
6. 承認資料と実施条件は本システム外で管理し、本アプリが検証したと表示しない。
