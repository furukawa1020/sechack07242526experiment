...
- screen-v2コードの最終5コマンド: 2026-07-22の最終候補作業ツリーで全件PASS
- 実参加者を対象とする本番: 6件の`goEvidence`と最終リリース照合が完了するまで**NO-GO**

次の5コマンドを、最終コード変更を含む同一の候補作業ツリーから実行した。正式成果物を封印するときは、`goEvidence`が参照する最終source commitでも再実行し、commit・source tree SHA-256・実行記録を外部管理票へ固定する。過去版の成功結果や部分実行はscreen-v2の合格記録として流用しない。
...
| コマンド | 結果 | 実測日時 |
| --- | --- | --- |
| `npm run lint` | PASS（エラー0件） | 2026-07-22 JST |
| `npm run typecheck` | PASS（型エラー0件） | 2026-07-22 JST |
| `npm test` | PASS（34 files、539 tests） | 2026-07-22 JST |
| `npm run test:e2e` | PASS（10 tests） | 2026-07-22 JST |
| `npm run build` | PASS（client/server） | 2026-07-22 JST |
...
補助的な画面回帰として`npm run test:public-demo`も実行し、5 viewport・25 testsがPASSした。この結果は正式productionの一般公開を意味せず、正式成果物のGO判定にも代用しない。
