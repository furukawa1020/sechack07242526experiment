...
| コマンド                             | 最終結果                                                             | 実行日時             |
| ------------------------------------ | -------------------------------------------------------------------- | -------------------- |
| `npm run lint`                       | 成功                                                                 | 2026-07-20 14:56 JST |
| `npm run typecheck`                  | 成功                                                                 | 2026-07-20 14:56 JST |
| `npm test`                           | 成功: 17ファイル、163テスト                                          | 2026-07-20 14:57 JST |
| `npm run test:e2e`                   | 成功: Chromium 8テスト                                               | 2026-07-20 14:58 JST |
| `npm run build`                      | 成功                                                                 | 2026-07-20 14:57 JST |
...
2026-07-20の読取り専用監査では、URLがHTTP 200を返し、想定した研究タイトルと一致することを確認した。一方、公開payloadに内部条件A〜Dの固定対応が含まれ、3種類と4種類の説明、各提示直後と4提示後の回答説明がそれぞれ併存していた。評価文として機械抽出できたのは10個であり、「11問」との一致は確認できていない。メール収集設定は機械判定していない。事実、監査限界、解除条件は[Googleフォーム公開内容監査](FORM_AUDIT.md)に記録した。
