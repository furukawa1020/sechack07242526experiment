...
2026-07-21にcommit `14d6a6069c30c63aa1d1561c7d8b6e9a9d94f2d0`を配信し、許可ファイル一覧とSHA-256、配信commit、全5経路のHTTP 200、実ブラウザ描画、同一ブラウザ内の表示同期、模擬装置操作を確認した。ローカル成果物は5画面幅・20ケースすべて成功し、公開HTTPSへ直接実行した1366×768の4ケースもすべて成功した。この実測中の外部originへのリクエストとWebSocketはともに0件だった。
