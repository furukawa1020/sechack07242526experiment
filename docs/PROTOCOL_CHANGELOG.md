...
## R8-010-2x2-screen-v3 — 2026-07-23

- A=cloud+label、B=local+label、C=local+puffer、D=cloud+pufferの4条件、提示順ABDC / BCAD / CDBA / DACB、固定値72 / 高ストレス / 0.60、8秒 / 3秒 / 15秒 / 7秒、A/BとC/Dの刺激同一性は変更していない。
- 各提示の`reset`完了後に、時間制限のない中立な`response`チェックポイントを追加した。第1〜第3提示後はOperatorの明示確認後だけ次の`handling`へ進み、第4提示後は同じ確認後だけ`summary`へ進む。
- 参加者画面のチェックポイント文言を`第{n}提示は終了しました`／`研究スタッフの案内をお待ちください。`へ固定し、フォーム名、URL、QRコード、回答内容、回答状況、内部条件コードおよび直前の結果刺激を表示しない。
