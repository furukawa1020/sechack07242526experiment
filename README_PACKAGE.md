...
このパッケージは、`R8-010-2x2-screen-v2`の実験提示サイトを作るための仕様書です。正式参加者UIと正式production成果物には、フォーム、外部アンケート、URL、QR、回答誘導、`FORM_*`、`MOCK_REHEARSAL`、`PUBLIC_DEMO`を含めません。USB機器と物理フグも不要です。`serial`による物理版は将来の別プロトコルです。開発用Mock設定と正式screen設定を混同しないでください。
