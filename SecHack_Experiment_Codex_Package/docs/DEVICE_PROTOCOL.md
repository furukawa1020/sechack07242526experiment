...
`R8-010-3condition-screen-v1`の正式MVPは`device.mode=screen`を使用し、条件Cで参加者画面内のフグだけを動かす。USBシリアル機器を接続せず、デバイス制御のための外部通信も行わない。
...
`mock`を正式実施の代替として使用してはならない。`serial`を`R8-010-3condition-screen-v1`へ混在させてはならない。物理フグと画面上フグの切替は研究刺激の変更であるため、研究責任者の承認と、所属機関で必要な倫理審査・変更手続きを完了してから別プロトコルとして実施する。
...
1. Cの`result`開始と同じサーバ時刻に、`level=0.60`、`rampMs=6000`の膨張を開始する。
...
条件Cの命令列、目標レベル、開始時刻、描画、膨張速度、保持時間、収縮速度を設定とサーバ時刻へ固定し、順序や参加者によって変えない。

サーバを唯一の時刻源とし、スナップショットにフェーズ開始時刻、終了予定時刻、サーバ現在時刻および画面フグ状態を含める。参加者画面はその時刻差から進捗を描画し、クライアント固有のタイマーだけで状態を決定しない。継続接続中の再描画は経過分を反映した同じ進捗から描画する。

`result`または`reset`中に参加者画面の再読み込み・切断を検出した場合は、ラベル条件かフグ条件かにかかわらず、A〜Cの全条件で次の順序を固定する。

1. 進行タイマーを破棄し、参加者向けスナップショットを直ちに`recoveryRequired`へ更新して、条件刺激を中立な案内へ置き換える。
2. `stop`を試行する。
3. `deflate`を試行し、成功時は`idle`かつ`level=0`を確認して`device.deflate.complete`を監査記録する。確認できない場合も失敗を監査記録し、以後の提示へは戻らない。
4. DEFLATEの完了確認または確認失敗の記録後に限り、セッションを`error`へ遷移させて`session.error`を記録する。

この経路のセッションは再接続しても復元・再開しない。`intro`、`handling`、`processing`または`summary`中の参加者画面喪失では、条件刺激を進めず中立な復旧表示へ移し、再接続後もOperatorの明示確認まで再開しない。

通常の中断、緊急停止またはアプリ終了時にも`stop`、続けて`deflate`を状態機械上で試行する。
...
以下は将来の物理フグ版で維持する通信・安全要件であり、`R8-010-3condition-screen-v1`の正式実施では使用しない。
...
条件Cのコマンド列が固定仕様と一致することをテストできるようにする。
